...
def mom_budg_hm(run, lev=150, filename='plev_pentad', timeav='pentad', period_fac=1.,lonin=[-1.,361.]):
    ...
    damping = data.dt_ug_diffusion.sel(lon=lons).mean('lon').sel(pfull=lev)*86400.
    ...
    totp.plot.contour(ax=ax1, x='xofyear', y='lat', extend='both', levels=np.arange(-92.,109.,100.), add_colorbar=False, add_labels=False, colors='0.7', linewidths=2)
    ...
    totp.plot.contour(ax=ax2, x='xofyear', y='lat', extend='both', levels=np.arange(-92.,109.,100.), add_colorbar=False, add_labels=False, colors='0.7', linewidths=2)
    ax2.grid(True,linestyle=':')
    ax2.set_ylim(-60,60)
    
    #Third plot
    dphidx.plot.contourf(ax=ax3, x='xofyear', y='lat', extend='both', levels = levels, add_colorbar=False, add_labels=False)
    totp.plot.contour(ax=ax3, x='xofyear', y='lat', extend='both', levels=np.arange(-92.,109.,100.), add_colorbar=False, add_labels=False, colors='0.7', linewidths=2)
    ax3.grid(True,linestyle=':')
    ax3.set_ylim(-60,60)
    
    #Fourth plot
    mom_trans.plot.contourf(ax=ax4, x='xofyear', y='lat', extend='both', levels = levels, add_colorbar=False, add_labels=False)
    totp.plot.contour(ax=ax4, x='xofyear', y='lat', extend='both', levels=np.arange(-92.,109.,100.), add_colorbar=False, add_labels=False, colors='0.7', linewidths=2)
    ...
    totp.plot.contour(ax=ax5, x='xofyear', y='lat', extend='both', levels=np.arange(-92.,109.,100.), add_colorbar=False, add_labels=False, colors='0.7', linewidths=2)
    ...
    #Sixth plot
    damping.plot.contourf(ax=ax6, x='xofyear', y='lat', extend='both', levels = levels, add_colorbar=False, add_labels=False)
    totp.plot.contour(ax=ax6, x='xofyear', y='lat', extend='both', levels=np.arange(-92.,109.,100.), add_colorbar=False, add_labels=False, colors='0.7', linewidths=2)
    ...
    if lonin == [-1.,361.]:
        figname = 'zon_mom_budg_low_' +run+ '.pdf'
    else:
        figname = 'zon_mom_budg_low_' + run + '_' + str(int(lonin[0]))+ '_' + str(int(lonin[1])) + '.pdf'
    
    plt.savefig(plot_dir + figname, format='pdf')
    plt.close()


mom_budg_hm('ap_20', lev=850)
mom_budg_hm('ap_2', lev=850)
mom_budg_hm('full_qflux', lev=850)
mom_budg_hm('full_qflux', lonin=[60.,150.], lev=850)
