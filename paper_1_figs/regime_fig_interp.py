...
def get_edge_psi(run, lonin=[-1.,361.], sanity_check=False, lev=lev):
    ...
    if sanity_check:
        ...
        psi_new.sel(pfull=lev).plot.contourf(levels=np.arange(-300.,301.,50.))
...
line_high_aps, consts_high = fit_power_law(edge_ll_aps, psi_min_aps, maxmin=[9.,40.])

line_low_fq, consts_low = fit_power_law(edge_ll_fq.values, psi_min_fq.values, maxmin=[0.,9.])
line_high_fq, consts_high = fit_power_law(edge_ll_fq.values, psi_min_fq.values, maxmin=[9.,40.])

plt.plot(np.arange(0.,10.), line_low_aps, 'k:', linewidth=2)
plt.plot(np.arange(9.,41.), line_high_aps, 'k:', linewidth=2)

plt.plot(np.arange(0.,10.), line_low_fq, 'r:', linewidth=2)
plt.plot(np.arange(9.,41.), line_high_fq, 'r:', linewidth=2)
...
plt.tight_layout()  
figname = 'psi_latvsstrength_' + str(int(lev)) + '_' + str(int(thresh)) + '_interp.pdf'
