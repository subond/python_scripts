...
cp = 287.04/2*7
L = 2.500e6
data_ap2['mse'] = (data_ap2.temp*cp + data_ap2.sphum*L + data_ap2.height*g).mean('lon')/1000.
data_ap20['mse'] = (data_ap20.temp*cp + data_ap20.sphum*L + data_ap20.height*g).mean('lon')/1000.
data_full['mse'] = (data_full.temp*cp + data_full.sphum*L + data_full.height*g).sel(lon=lons).mean('lon')/1000.
...
cs = data_full.mse.sel(pfull=850.).plot.contour(ax=ax1, x='xofyear', y='lat', levels=np.arange(200.,401.,10.), add_labels = False, colors='0.7', add_colorbar=False)
...
f1 = data_ap2.totp.plot.contourf(ax=ax2, x='xofyear', y='lat', levels = plevels, add_colorbar=False, add_labels=False, extend='max', cmap='Blues')
data_ap2.totp.plot.contour(ax=ax2, x='xofyear', y='lat',levels=np.arange(-92.,109.,100.), add_labels = False, add_colorbar=False, colors='k')
cs = data_ap2.mse.sel(pfull=850.).plot.contour(ax=ax2, x='xofyear', y='lat', levels=np.arange(200.,401.,20.), add_labels = False, colors='0.7', add_colorbar=False)
plt.clabel(cs, fontsize=15, inline_spacing=-1, fmt= '%1.0f')
...
f1 = data_ap20.totp.plot.contourf(ax=ax3, x='xofyear', y='lat', levels = plevels, add_colorbar=False, add_labels=False, extend='max', cmap='Blues')
data_ap20.totp.plot.contour(ax=ax3, x='xofyear', y='lat',levels=np.arange(-92.,109.,100.), add_labels = False, add_colorbar=False, colors='k')
cs = data_ap20.mse.sel(pfull=850.).plot.contour(ax=ax3, x='xofyear', y='lat', levels=np.arange(200.,401.,10.), add_labels = False, colors='0.7', add_colorbar=False)
plt.clabel(cs, fontsize=15, inline_spacing=-1, fmt= '%1.0f')
