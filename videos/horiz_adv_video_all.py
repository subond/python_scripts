...
plot_dir = '/scratch/rg419/plots/horiz_adv_video/'
mkdir = sh.mkdir.bake('-p')
mkdir(plot_dir)


def horiz_adv_plot(run, ax_in, pentad, plot_land = True):
    ...
    f1 = horiz_md_mean[pentad,:,:].plot.contourf(x='lon', y='lat', levels = np.arange(-5.,5.1,1.), ax=ax_in, add_labels = False, extend='both', add_colorbar=False)
    if plot_land:
...
for pentad in range(0,72):
    ...
    horiz_adv_plot('ap_2', ax1, pentad)
    horiz_adv_plot('ap_20', ax2, pentad)
    horiz_adv_plot('ap_20_qflux', ax3, pentad)
    horiz_adv_plot('am_qflux', ax4, pentad)
    horiz_adv_plot('flat_qflux', ax5, pentad)
    f1 = horiz_adv_plot('full_qflux', ax6, pentad)
    ...
    plot_name = plot_dir+'horiz_adv_pentad_%02d.png' % (pentad+1)
