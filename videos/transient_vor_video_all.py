...
plot_dir = '/scratch/rg419/plots/transient_vor_video/'
...
def transient_plot(run, ax_in, pentad, plot_land = True):
    ...
    transient = (data_vort.horiz_md.sel(pfull=150).values + data_vort.stretching.sel(pfull=150).values)* 86400.**2. - horiz_md_mean - stretching_mean
    
    land_file = '/scratch/rg419/GFDL_model/GFDLmoistModel/input/land.nc'
    land = xr.open_dataset( land_file)
    data['land'] = (('lat','lon'), land.land_mask)
    
    f1 = transient[pentad,:,:].plot.contourf(x='lon', y='lat', levels = np.arange(-5.,5.1,1.), ax=ax_in, add_labels = False, extend='both', add_colorbar=False)
...
for pentad in range(0,72):
    ...
    transient_plot('ap_2', ax1, pentad)
    #transient_plot('ap_20', ax2, pentad)
    transient_plot('ap_20_qflux', ax3, pentad)
    transient_plot('am_qflux', ax4, pentad)
    transient_plot('flat_qflux', ax5, pentad)
    f1 = transient_plot('full_qflux', ax6, pentad)
    ...
    plot_name = plot_dir+'trans_vor_pentad_%02d.png' % (pentad+1)
