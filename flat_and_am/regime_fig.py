...
edge_ll_ft, psi_min_ft = get_edge_psi('flat_qflux', lonin=[60.,150.])
edge_ll_am, psi_min_am = get_edge_psi('am_qflux', lonin=[60.,150.])
...
line_low_fq, consts_low = fit_power_law(edge_ll_fq, psi_min_fq, maxmin=[0.,9.])
line_high_fq, consts_high = fit_power_law(edge_ll_fq, psi_min_fq, maxmin=[9.,30.])

line_low_ft, consts_low = fit_power_law(edge_ll_ft, psi_min_ft, maxmin=[0.,9.])
line_high_ft, consts_high = fit_power_law(edge_ll_ft, psi_min_ft, maxmin=[9.,30.])

line_low_am, consts_low = fit_power_law(edge_ll_am, psi_min_am, maxmin=[0.,9.])
line_high_am, consts_high = fit_power_law(edge_ll_am, psi_min_am, maxmin=[9.,30.])
...
plt.plot(np.arange(0.,10.), line_low_ft, 'r:', linewidth=2)
plt.plot(np.arange(9.,31.), line_high_ft, 'r:', linewidth=2)

plt.plot(edge_ll_20, psi_min_20, 'bx', ms=10, mew=2)
#plt.plot(edge_ll_fq, psi_min_fq, 'rx', ms=10, mew=2)
plt.plot(edge_ll_ft, psi_min_ft, 'rx', ms=10, mew=2)
...
plt.tight_layout()  
figname = 'psi_latvsstrength_ft_' + str(int(lev)) + '_' + str(int(thresh)) + '.pdf'
