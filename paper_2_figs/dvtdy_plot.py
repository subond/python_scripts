...
def vdtdy_plot(run, lev=150.):
    ...
    dTdt = (gr.ddt(theta)).mean('lon').sel(pfull=lev)*86400.
    ...
    f1 = dvtdy.plot.contourf(ax=ax1, x='xofyear', y='lat', add_labels=False, levels=np.arange(-100.,101.,10.), extend='both')
    ...
    f1 = dwtdp.plot.contourf(ax=ax2, x='xofyear', y='lat', add_labels=False, levels=np.arange(-100.,101.,10.), extend='both')
    ...
    f1 = (dvtdy+dwtdp).plot.contourf(ax=ax3, x='xofyear', y='lat', add_labels=False, levels=np.arange(-50.,51.,5.), extend='both')
   ...
   # cb1.set_label('Advective temperature tendency, Kday$^{-1}$')
    ...
    plt.savefig(plot_dir+'dvtdy_150_' + run + '.pdf', format='pdf')
    plt.close()
