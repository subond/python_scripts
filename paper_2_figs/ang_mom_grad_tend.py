"""
Plot the tendency of UgradM (9/11/2018)
"""
...
def ang_mom_grad_plot(run, rot_fac=1., lev=150.):
    ...
    vor = (-1.* u_dy + f).sel(pfull=lev).mean('lon')*86400.
    
    vertical_term = (-1.* u_dp).sel(pfull=lev).mean('lon')*86400.
    
    v = data.vcomp.sel(pfull=lev).mean('lon')
    w = data.omega.sel(pfull=lev).mean('lon')
    
    # normalise so that if vor is parallel to M, value is 0, if perpendicular, 1
    u_ang_mom_grad = (vor * v + vertical_term * w) / np.sqrt(v**2. + w**2.) / np.sqrt(vor**2. + vertical_term**2.)
    ...
    f1 = u_ang_mom_grad.plot.contourf(ax=ax1, x='xofyear', y='lat', levels=np.arange(-1.,1.1,0.1), add_colorbar=False, add_labels=False, extend='both')
    ...
    #cb1.set_label('$U \cdot \nabla M')
    
    plt.savefig(plot_dir+'u_ang_mom_grad_' + run + '.pdf', format='pdf')
    plt.close()
...
if __name__ == "__main__":
    ...
    ang_mom_grad_plot('sn_1.000_evap_fluxes_heattrans')
