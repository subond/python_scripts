...
def evap_parts(run, lonin=[-1.,361.], do_make_sym=True):
    ...
    rho_a.plot.contourf(x='xofyear', y='lat', ax=ax2, extend = 'both', add_labels=False, levels=np.arange(1.12,1.26,0.01))
    ...
    plt.savefig(plot_dir + 'evap_parts_revisions_' + run + '.pdf', format='pdf')
    plt.close()
