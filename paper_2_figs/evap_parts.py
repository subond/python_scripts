...
def evap_parts(run, lonin=[-1.,361.]):
    ...
    qdiff.plot.contourf(x='xofyear', y='lat', levels=np.arange(-0.0045,0.0045,0.0005), ax=ax1, extend = 'both', add_labels=False)
    ax1.set_title('q_a - q_s')
    
    v_a.plot.contourf(x='xofyear', y='lat', levels=np.arange(0.,20.,2.),  ax=ax2, extend = 'both', add_labels=False)
    ax2.set_title('v_a')
    
    rho_a.plot.contourf(x='xofyear', y='lat', levels=np.arange(1.,1.4,0.02), ax=ax3, extend = 'both', add_labels=False)
    ax3.set_title('rho_a')
