...
def mombudg_2d_mn_fn(inp_fol,nproc,years):
...
def mombudg_2d_pd_fn(inp_fol,nproc,years, trange):
...
#nproc   = no of processors
    ...
    #Initialise arrays to load into
    u =     xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    v =     xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    w =     xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    uu =    xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    uv =    xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    uw =    xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    phi =   xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    ddamp = xr.DataArray(np.zeros((trl,17,64,128,len(years))), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', rundata.pfull), ('lat', rundata.lat), ('lon', rundata.lon), ('year', years )])
    ...
    #Evaluate gradients needed
    dudx_av = xr.DataArray( cfd(u_av.values,u_av.lon*np.pi/180,3),   [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', u_av.pfull ), ('lat', u_av.lat), ('lon', u_av.lon )])
    ududx_av = u_av*dudx_av/coslat/a
    dudy_av = xr.DataArray( cfd((u_av*coslat).values,u_av.lat*np.pi/180,2),   [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', u_av.pfull ), ('lat', u_av.lat), ('lon', u_av.lon )])
    vdudy_av = v_av*dudy_av/coslat/a
    dudp_av = xr.DataArray( cfd(u_av.values,u_av.pfull*100,1), [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', u_av.pfull ), ('lat', u_av.lat), ('lon', u_av.lon )])
    wdudp_av = w_av*dudp_av

    duueddx_av = xr.DataArray( cfd(uued_av.values,uued_av.lon*np.pi/180,3),   [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', uued_av.pfull ), ('lat', uued_av.lat), ('lon', uued_av.lon )])
    duueddx_av = duueddx_av/coslat/a
    duveddy_av = xr.DataArray( cfd(uved_av.values ,uved_av.lat*np.pi/180,2),   [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', uued_av.pfull ), ('lat', uued_av.lat), ('lon', uued_av.lon )])
    duveddy_av = duveddy_av/coslat/coslat/a
    duweddp_av = xr.DataArray( cfd(uwed_av.values,uwed_av.pfull*100,1),   [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', uued_av.pfull ), ('lat', uued_av.lat), ('lon', uued_av.lon )])

    dphidx_av = xr.DataArray( cfd(phi_av.values,phi_av.lon*np.pi/180,3),   [('pentad', mngrp.pentad.values[trange[0]:trange[1]] ), ('pfull', phi_av.pfull ), ('lat', phi_av.lat), ('lon', phi_av.lon )])
    dphidx_av = -1*dphidx_av/coslat/a
