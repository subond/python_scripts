...
name_temp = '/scratch/rg419/Data_moist/full_qflux_bucket_scale/run%03d/atmos_monthly.nc'
...
djf = data.isel(time = range(0,2) + range(11,12)).mean('time')
mam = data.isel(time = range(2,5)).mean('time')
jja = data.isel(time = range(5,8)).mean('time')
son = data.isel(time = range(8,11)).mean('time')
...
plt.savefig(plot_dir+'precip_test_bucket_scale.pdf', format='pdf')
plt.close()
...
plt.savefig(plot_dir+'evap_test_bucket_scale.pdf', format='pdf')
plt.close()
