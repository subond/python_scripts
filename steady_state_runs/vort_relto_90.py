...
def get_abs_vort(run):
    ...
    data_90 = xr.open_dataset('/scratch/rg419/Data_moist/climatologies/vort_eq_ss_90.000.nc')

    # Calculate vertical component of absolute vorticity = f + dv/dx - du/dy
    v_dx_90 = gr.ddx(data_90.vcomp)  # dvdx
    u_dy_90 = gr.ddy(data_90.ucomp)  # dudy
    vor_90 = v_dx_90 - u_dy_90 + f
    
    abs_vort_90 = vor_90*86400.
    
    abs_vort_diff = abs_vort.sel(pfull=150) - abs_vort_90.sel(pfull=150)
    
    return abs_vort_diff
...
for i in range(18):
    ...
    abs_vort_diff = get_abs_vort(run)
    ...
    f1 = abs_vort_diff.plot.contourf(x='lon', y='lat', levels=np.arange(-5.,5.2,0.5), ax=ax, extend = 'both', add_labels=False, add_colorbar=False)
