...
def get_transient(run):
    ...
    transient = data.horiz_md.sel(pfull=150.) + data.stretching.sel(pfull=150.) - horiz_adv_mean - stretching_mean
...
for i in range(18):
    ...
    f1 = transient.plot.contourf(x='lon', y='lat', levels = np.arange(-5.,5.1,0.5), ax=ax, extend = 'both', add_labels=False, add_colorbar=False)
...
figname = 'vort_eddies_qflux_ll.pdf'
