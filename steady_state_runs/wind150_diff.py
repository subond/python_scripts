...
def get_wind(run1, run2):
    ...
    u = data_1.ucomp.sel(pfull=150)
    v = data_1.vcomp.sel(pfull=150)
    omega = (data_1.omega.sel(pfull=500))*86400./100. # Convert to hPa/day
    
    u_diff = data_2.ucomp.sel(pfull=150) - u
    v_diff = data_2.vcomp.sel(pfull=150) - v
    omega_diff = data_2.omega.sel(pfull=500)*86400./100.  - omega
        
    return u_diff, v_diff, omega_diff, u, v, omega
...
def plot_diff(diff_type = 'strength'):
    ...
    for i in range(18):
        ...
        u_diff, v_diff, omega_diff, u, v, omega = get_wind(run_1, run_2)
        ...
        if (diff_type =='strength' and i in range(0,18,3)) or (diff_type =='loc' and i in range(3)):
            ...
            b = ax.quiver(u.lon[::5], v.lat[::2], u[::2,::5], v[::2,::5], scale=1000., angles='xy')#,headwidth=5)
        else:
            f2 = omega_diff.plot.contourf(x='lon', y='lat', levels=np.arange(-120.,121.,10.), ax=ax, extend = 'both', add_labels=False, add_colorbar=False)
            b = ax.quiver(u_diff.lon[::5], v_diff.lat[::2], u_diff[::2,::5], v_diff[::2,::5], scale=1000., angles='xy')#,headwidth=5)
    ...
    if diff_type == 'strength':
        ...
        cb1=f.colorbar(f2, ax=(ax_sublist), use_gridspec=True, orientation = 'horizontal',fraction=0.05, pad=0.07, aspect=30, shrink=0.5)
        cb2=f.colorbar(f1, ax=([ax1,ax2,ax3]), use_gridspec=True, orientation = 'horizontal',fraction=0.05, pad=0.07, aspect=30, shrink=0.5)
    
    for label in cb2.ax.xaxis.get_ticklabels()[1::2]:
        label.set_visible(False)
            
    figname = 'uv_150_w_500_diff_' + diff_type + '.pdf'
