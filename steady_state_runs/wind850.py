...
def get_abs_vort(run):
    ...
    u = data.ucomp.sel(pfull=850)
    v = data.vcomp.sel(pfull=850)
    sst  = data.t_surf
...
for i in range(18):
    ...
    b = ax.quiver(u.lon[::5], v.lat[::2], u[::2,::5], v[::2,::5], scale=200., angles='xy')#, scale=500.,headwidth=5)
...
figname = 'uv_850_sst_qflux_ll_full.pdf'
