# Plot total vorticity tendency (lat-time)
...
plt.set_cmap('RdBu_r')
#First plot
f1 = (86400.**2.*data_180.total).sel(pfull=lev).mean('lon').plot.contourf(ax=ax1, x='pentad', y='lat', levels = levels, add_colorbar=False, add_labels=False, extend='both')
...
f1 = (86400.**2.*data_360.total).sel(pfull=lev).mean('lon').plot.contourf(ax=ax2, x='pentad', y='lat', levels = levels, add_colorbar=False, add_labels=False, extend='both')
...
f1 = (86400.**2.*data_720.total).sel(pfull=lev).mean('lon').plot.contourf(ax=ax3, x='pentad', y='lat', levels = levels, add_colorbar=False, add_labels=False, extend='both')
...
plt.savefig(plot_dir+'vort_total.pdf', format='pdf')
plt.close()        
