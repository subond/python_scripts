...
def vort_eq_hm(run, lev=150, period_fac=1.):
    ...
    abs_vort = vor.mean('lon').sel(pfull=lev)*86400.
    ...
    data['transient'] = (('pentad','pfull','lat','lon'), transient )	
    
    horiz_md_hm = horiz_md_mean.mean('lon').sel(pfull=lev)
    stretching_hm = stretching_mean.mean('lon').sel(pfull=lev)
    transient_hm = data.transient.mean('lon').sel(pfull=lev)
    total_hm = data.total.sel(pfull=lev).mean('lon')*86400.**2.
    ...
    plt.set_cmap('RdBu_r')
    #First plot
    f2=horiz_md_hm.plot.contourf(x='pentad', y='lat', levels=levels, ax=ax1, extend = 'both', add_labels=False, add_colorbar=False)
    ax1.contour(data.pentad, data.lat, abs_vort.T, levels=np.arange(-12.,13.,2.), colors='k', linewidths=2, alpha=0.25)
    ...
    #Second plot
    stretching_hm.plot.contourf(x='pentad', y='lat', levels=levels, ax=ax2, extend = 'both', add_labels=False, add_colorbar=False)
    ax2.contour(data.pentad, data.lat, abs_vort.T, levels=np.arange(-12.,13.,2.), colors='k', linewidths=2, alpha=0.25)
    ax2.set_ylim(-60,60)
    ...
    ax3.contour(data.pentad, data.lat, abs_vort.T, levels=np.arange(-12.,13.,2.), colors='k', linewidths=2, alpha=0.25)
    ax3.set_ylabel('Latitude')
    ax3.set_ylim(-60,60)
    ax3.set_yticks(np.arange(-60,61,30))
    ax3.set_xticks(tickspace * period_fac)
    ...
    ax4.contour(data.pentad, data.lat, abs_vort.T, levels=np.arange(-12.,13.,2.), colors='k', linewidths=2, alpha=0.25)
    ax4.set_ylim(-60,60)
    ax4.set_yticks(np.arange(-60,61,30))
    ax4.set_xticks(tickspace * period_fac)
