''' 
10/10/2018 Make GPCP hms of different monsoon regions:
- East Asia
- Western Pacific
- India
- West Africa
...
def precip_plot(data, ax, lons, title, levels=np.arange(0.,15.,2.), sh=False):
    ...
    if sh:
        precip = xr.DataArray(data.precipitation.sel(lon=lons).mean('lon').values, [('xofyear', (np.arange(1,74)-37)%73-36), ('lat', data.lat)])
        precip = precip.sortby('xofyear')
        f1 = precip.plot.contourf(ax=ax, x='xofyear', y='lat', levels = levels, add_labels=False, extend='max', cmap='Blues', add_colorbar=False)
    else:
...
f1 = precip_plot(data_gpcp, ax1, [60.,80.], 'India')
precip_plot(data_gpcp, ax2, [90.,100.], 'Bay of Bengal')
precip_plot(data_gpcp, ax3, [110.,140.], 'East Asia')
precip_plot(data_gpcp, ax4, [15.,50.], 'South Africa', sh=True)
precip_plot(data_gpcp, ax5, [140.,160.], 'Western North Pacific')
precip_plot(data_gpcp, ax6, [-120.,-80.], 'North America')
precip_plot(data_gpcp, ax7, [-20.,20.], 'West Africa')
precip_plot(data_gpcp, ax8, [-80.,-40.], 'South America', sh=True)
...
# Save as a pdf
plt.savefig(plot_dir + 'precip_monsoons_gpcp.pdf', format='pdf')
plt.close()
