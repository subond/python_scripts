...
def correlate_onset(monsoon, var, varno, levs=[20000.], years=range(1958,2017)):
    ...
    if monsoon == 'SCS':
        onsets = np.load('/scratch/rg419/python_scripts/onset_variability/jra_onsets_scsm.npy')
        onsets = onsets * 5. - 2.5
    elif monsoon == 'BOB':
        onsets = np.load('/scratch/rg419/python_scripts/onset_variability/jra_onsets_bob_days.npy')
        onsets = np.array(onsets)
    elif monsoon == 'ISM':
        onsets = np.load('/scratch/rg419/python_scripts/onset_variability/jra_onsets_ism_days.npy')
        onsets = np.array(onsets)
    else:
        print('Invalid input')
        return
    ...
    # Load in JRA data
    data = xr.open_dataset('/disca/share/reanalysis_links/jra_55/1958_2016/' + var + '_monthly/atmos_monthly_together.nc')
    data = data[varno].load().loc['1958-01':'2017-12']  
    ...
    for lev in levs:
        try:
            correl_plot = correl.sel(lev=lev)
        except:
            correl_plot = correl
        ...
        i=0
        for ax in axes:
            f1 = correl_plot[i,:,:].plot.contourf(ax=ax, x='lon',y='lat', levels=np.arange(-1.,1.1,0.25), add_labels=False, add_colorbar=False)
            #correl[i,:,:].plot.contour(ax=ax, x='lon',y='lat', levels=np.arange(-100.4,100.,100.), colors='k', lw=2, add_labels=False)
            #correl[i,:,:].plot.contour(ax=ax, x='lon',y='lat', levels=np.arange(-99.6,100.,100.), colors='k', lw=2, add_labels=False)
        ...
        plt.savefig(plot_dir + 'jra_corr_' + monsoon + '_' + var + '_' + str(lev) + '.pdf', format='pdf')
        plt.close()
...
for var in [['ucomp','var33'], ['vcomp','var34'], ['omega', 'var39'], ['height', 'var7'], ['temp', 'var11'], ['slp', 'var2']]:
    correlate_onset('BOB', var[0], var[1], levs=np.arange(5000.,100001.,5000.))
    correlate_onset('SCS', var[0], var[1],levs=np.arange(5000.,100001.,5000.))
    correlate_onset('ISM', var[0], var[1],levs=np.arange(5000.,100001.,5000.))
