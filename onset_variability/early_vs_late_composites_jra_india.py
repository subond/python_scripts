'''25/09/2018 Identify early/late/normal onset timing for the different monsoons, and make a climatology of fields for these years'''
...
def early_vs_late_onset(var, varno, levs=[20000.], years=range(1958,2017), levels=np.arange(-6.,6.1,0.5)):
    # Load in onset dates
    ...
    data = data[varno].load().loc['1958-01':'2016-12']  
    ...
    def plot_by_timing(timing_coord, timing, levels=levels):
        ...
        for lev in levs:
            try:
                data_timing = data_timing.sel(lev=lev)
            except:
            ...
            i=0
            for ax in axes:
                f1 = data_timing[i,:,:].plot.contourf(ax=ax, x='lon',y='lat', add_labels=False, add_colorbar=False, levels=levels, extend='both')
        ...
        plt.savefig(plot_dir + 'jra_' + timing + '_' + var + '_' + str(lev) + '.pdf', format='pdf')
        plt.close()
   ...
   # return correl
for var in [['ucomp','var33', np.arange(-5.,5.1,0.5)], ['vcomp','var34', np.arange(-5.,5.1,0.5)], 
                      ['omega', 'var39', np.arange(-0.03,0.031,0.005)], ['height', 'var7', np.arange(-60.,61.,5.)], 
                      ['temp', 'var11', np.arange(-1.2,1.21,0.2)], ['slp', 'var2', np.arange(-500.,501.,50.)]]:
    early_vs_late_onset(var[0], var[1], levels=var[2])
