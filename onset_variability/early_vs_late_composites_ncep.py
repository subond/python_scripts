'''3/10/2018 Identify early/late/normal onset timing for the different monsoons, and make a climatology of fields for these years using NCEP/NCAR data'''
...
def early_vs_late_onset(var, levs=[200], years=range(1948,2017), levels=np.arange(-6.,6.1,0.5)):
    # Load in onset dates
    onsets_scsm = np.load('/scratch/rg419/python_scripts/onset_variability/ncep_onsets_scsm.npy')
    onsets_bob  = np.load('/scratch/rg419/python_scripts/onset_variability/ncep_onsets_bob_days.npy')
    onsets_ism  = np.load('/scratch/rg419/python_scripts/onset_variability/ncep_onsets_ism_days.npy')
    ...
    # Load in NCEP/NCAR data
    data = xr.open_mfdataset('/disca/share/reanalysis_links/NCEP_NCAR/' + var + '.mon.mean.nc')
    data = data[var].load().loc['1948-01':'2016-12']
    ...
    def plot_by_timing(timing_coord, timing, levels=levels):
        ...
        for lev in levs:
            ...
            i=0
            for ax in axes:
                f1 = data_timing[i,:,:].plot.contourf(ax=ax, x='lon',y='lat', add_labels=False, add_colorbar=False, extend='both',levels=levels)
        ...
        plt.savefig(plot_dir + 'ncep_' + timing + '_' + var + '_' + str(lev) + '.pdf', format='pdf')
        plt.close()
...
var_list = [['uwnd',np.arange(-5.,5.1,0.5)],
        ['vwnd',np.arange(-5.,5.1,0.5)],
        ['air',np.arange(-1.2,1.21,0.2)],
        ['shum',np.arange(-1.e-5,1.e-5,1.e-6)],
        ['omega',np.arange(-0.03,0.031,0.005)],
        ['hgt',np.arange(-65.,66.,5.)]]
