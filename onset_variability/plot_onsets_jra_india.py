...
plt.fill_between([1958,2016], [round(ism_stats[0]) - 0.75*ism_stats[1], round(ism_stats[0]) - 0.75*ism_stats[1]], 
                              [round(ism_stats[0]) + 0.75*ism_stats[1], round(ism_stats[0]) + 0.75*scsm_stats[1]], alpha=0.2, color='C0')
plt.xlabel('Year')
...
def plot_winter_climate(data, title, levels_clim=np.arange(-50.,51.,5.), levels=np.arange(-5.,5.1,0.5), local=False):
    ...
    if local:
        plt.savefig('/scratch/rg419/plots/onset_variability_new/early_vs_late/JFM_' + title + '_local_ism.pdf', format='pdf')
    else:
        plt.savefig('/scratch/rg419/plots/onset_variability_new/early_vs_late/JFM_' + title + '_ism.pdf', format='pdf')
    plt.close()
