...
# Input profiles 
profiles = loadmat('/scratch/rg419/python_scripts/radiation_scheme/q_and_t_sbdart.mat')
...
def sw_geen():
    ...
    # Ref: Geen et al. 2016 (supplementary information)
...
def lw_down_geen():
    ...
    # Use optical depths to calculate LW in window and non window bands
    for k in range(0, n):
...
# Use LW and SW fluxes to calculate heating (K/s) of each layer
for k in range(0, n):
    # Total heating
...
# Plot total radiative heating rate in K/day
cs = (tdt_rad*86400.).plot.contour(x='lat', y='pfull', yincrease = False, levels = np.arange(-10,10,0.5), colors='k')
plt.clabel(cs, fmt='%1.1f')
plt.xlabel('Latitude')
plt.ylabel('Pressure, Pa')
plt.title('Total radiative heating')
plt.show()
