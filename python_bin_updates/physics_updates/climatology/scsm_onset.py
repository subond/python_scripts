...
def scsm_onset(u, pentaddim='xofyear', pdim='pfull', latdim='lat', londim='lon', print_onset=False):
    ...
    #print(np.round(u_mean.sel(pentad=pentads).values,1))
    #u_mean = u.sel(lon=lons).sel(lat=lats).mean(('lat','lon'))
    ...
    for i in range(len(pentads)):  # For each pentad
        ...
        if ((u_mean.sel(pentad=pentads[i]).values) > 0.  # Check if u_mean is greater than zero
            # and (all(u_mean.sel(pentad=pentads[i:i+4]).cumsum('pentad').values/np.arange(1.,5.) >= 1.))
...
if __name__ == "__main__":
    ...
    np.save('isca_era_onsets_scsm',np.array([onset_isca, onset_era]))
