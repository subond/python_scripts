""" 2/08/2018 Function to calculate the Wang 2009 Indian summer monsoon onset diagnostic.
...
def ism_onset(u, daydim='xofyear', pdim='pfull', latdim='lat', londim='lon', print_onset=False):
    ...
    lons = [u.lon[i].values for i in range(len(u.lon)) if u.lon[i] >= 40. and u.lon[i] <= 80.]
    days = [u.day[i].values for i in range(len(u.day)) if u.day[i] >=  130]
    ...
    for i in range(len(days)):  # For each day
        if (all(u_mean.sel(day=days[i:i+7]).values > 6.2)):  # Check if u_mean is greater than 6.2m/s on that day and the next 7
            onset_day = days[i]  # If that is true, that's your onset day
            if print_onset:
