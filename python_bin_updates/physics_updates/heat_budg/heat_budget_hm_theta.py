'''01/11/2018 Plot up terms contributing to potential temperature budget at a given level for an aquaplanet'''
...
#    diabatic = ((heating - data.omega/rho/mc.cp_air)*convTtotheta).sel(pfull=lev).mean('lon')*86400.
    diabatic = ((heating)*convTtotheta).sel(pfull=lev).mean('lon')*86400.
    
    #diabatic = tdt_rad + tdt_conv + tdt_cond + tdt_diff
    eddies = uT_dx_eddy + vT_dy_eddy + wT_dp_eddy
    
    heat_sum = uT_dx + vT_dy + wT_dp + diabatic + eddies
    ...
    ax2.set_title('Vertical advection', fontsize=17)
    ...
    figname = 'heat_budg_theta_' +run+ '.pdf'
